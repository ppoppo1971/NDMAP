...
- **가능 여부**: Data 레이어 `setStyle()`은 점선(dash)을 지원하지 않음. **가능**한 방법은 VMAP처럼 **Polyline + Symbol**(원을 일정 간격 반복)으로 그리는 것. 해당 방식으로 두께가 0 초과인 폴리선을 점선으로 표시 구현함.
- **두께 판별**: DXF `constantWidth`(그룹코드 43) > 0 또는 `lineweight`(370) > 0 이면 "두꺼운 선"으로 간주하고 점선 Polyline으로 그림.
- **다른 구별 방안**: 점선 대신 (1) **strokeWeight**만 키워서 두꺼운 선으로 표시, (2) **strokeOpacity**를 다르게, (3) 레이어/이름으로 필터해 별도 스타일 적용 등이 가능함. 선호 시 옵션으로 전환 가능.
