...
## DXF 좌표계 (도면과 지도 위치 맞추기)

DXF 도면이 **평면직각좌표**(지적도·측량 도면 등)이면, 단순 원점 보정만으로는 지도와 위치가 맞지 않습니다. **EPSG 코드**로 도면 좌표계를 지정하면 proj4로 WGS84에 맞게 변환합니다.

- **config.js** 의 `DXF_CRS` 에 도면에 해당하는 값을 넣으세요.
  - **EPSG:5186** (기본): 한국 2000 중부원점, y_0=600000 (국토지리정보원 표준). **y가 음수인 경우도 있음.**
  - **EPSG:5181**: 중부원점 y_0=500000 (카카오맵 등)
  - **EPSG:5179**: UTM-K 통합좌표계 (네이버지도 등)
- 도면/메타데이터의 x, y가 **미터 단위 평면직각좌표(E, N)** 이면 위 중 하나를 선택해 보세요. 여전히 안 맞으면 제작처에 도면 좌표계(EPSG 또는 원점·투영 정보)를 문의해 다른 코드로 바꿀 수 있습니다.
- **원점 보정만 쓰려면** (단위 m, 도면 원점을 특정 위경도에 두는 방식) `DXF_CRS = null` 로 두고 `MAP_ORIGIN_LAT`, `MAP_ORIGIN_LNG`, `DXF_UNITS_PER_METER` 만 사용하면 됩니다.

## 구성

- **config.js** – API 키, **DXF_CRS(EPSG)** / 원점·단위, 배경 없음 스타일
